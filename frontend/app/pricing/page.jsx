...
export default function PricingPage() {
  return (
    <div className="min-h-screen bg-[var(--black)] overflow-x-hidden transition-colors duration-300">
      <Navbar />
      ...
      <main className="pt-32 pb-24">
        <div className="container-shell">
          <div className="text-center mb-16">
            ...
            <h1 className="text-4xl md:text-6xl font-serif font-light text-[var(--white)] mb-6">
          ...
          <div className="mt-32 max-w-5xl mx-auto px-4">
            <h2 className="text-2xl md:text-3xl font-serif text-[var(--white)] text-center mb-12">Compare Features</h2>
            
            <div className="overflow-x-auto border border-[var(--line)] rounded-2xl bg-[var(--card)]">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-[var(--line)]">
                    <th className="p-6 text-sm font-bold text-[var(--muted)] uppercase tracking-wider">Feature</th>
                    <th className="p-6 text-sm font-bold text-[var(--white)] uppercase tracking-wider text-center">Free</th>
                ...
                <tbody>
                  {features.map((feature, i) => (
                    <tr key={i} className={`border-b border-[var(--line)] last:border-0 hover:bg-[var(--line)]/30 transition-colors`}>
                      <td className="p-6 text-[var(--white)]/80 font-medium">{feature.name}</td>
                      <td className="p-6 text-center text-sm text-[var(--muted)]">
                        {typeof feature.free === 'boolean' ? (
                          feature.free ? <Check className="w-5 h-5 text-accent mx-auto" /> : <Minus className="w-5 h-5 text-[var(--white)]/10 mx-auto" />
                        ) : feature.free}
                      </td>
                      <td className="p-6 text-center text-sm text-[var(--white)] bg-accent/[0.02]">
                        {typeof feature.premium === 'boolean' ? (
                          feature.premium ? <Check className="w-5 h-5 text-accent mx-auto" /> : <Minus className="w-5 h-5 text-[var(--white)]/10 mx-auto" />
                        ) : feature.premium}
                      </td>
                      <td className="p-6 text-center text-sm text-[var(--white)]">
                        {typeof feature.pro === 'boolean' ? (
                          feature.pro ? <Check className="w-5 h-5 text-blue-400 mx-auto" /> : <Minus className="w-5 h-5 text-[var(--white)]/10 mx-auto" />
                        ) : feature.pro}
                      </td>
                    </tr>
                  ))}
          ...
          <div className="mt-24 text-center">
            <h3 className="text-xl text-[var(--white)] mb-4">Have questions?</h3>
            <p className="text-[var(--muted)] mb-8">We're here to help you choose the right path.</p>
